...
function jok($data){ echo json_encode($data, JSON_UNESCAPED_UNICODE); exit; }

// Helpers catálogos permitidos
...
switch ($action) {
  ...
  case 'list_respuestas': {
    $sql = "SELECT numero,I,II,III,IV,V,VI,VII,VIII,IX,X,XI,XII,XIII,XIV,XV,XVI,XVII FROM respuestas ORDER BY numero ASC";
    $res = $mysqli->query($sql);
    $rows = [];
    while($row = $res->fetch_assoc()){ 
      // Force ints
      foreach (['numero','III','V','VI','VII','VIII','IX','X','XI','XII','XIII','XIV','XV','XVI','XVII'] as $k) { $row[$k] = intval($row[$k]); }
      $rows[] = $row;
    }
    jok($rows);
  }
  ...
  case 'create_respuesta': {
    ...
    $numero = $stmt->insert_id;
    $body['numero'] = intval($numero);
    // Normalizamos ints en la respuesta
