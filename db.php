...
switch ($action) {
    ...
    case 'list_respuestas': {
        $res = $mysqli->query("SELECT * FROM respuestas ORDER BY numero ASC");
    ...
    case 'create_respuesta': {
        ...
        $required = ['I','II','III','IV','V','VI','VII','VIII','IX','X','XI','XII','XIII','XIV','XV','XVI','XVII'];
        foreach($required as $f) {
            if(!isset($body[$f])) jerr("Falta el campo $f");
        }
        ...
        $VI   = intval($body['VI']);
        $VII  = intval($body['VII']);
        $VIII = intval($body['VIII']);
        $IX   = intval($body['IX']);
        $X    = intval($body['X']);
        $XI   = intval($body['XI']);
        $XII  = intval($body['XII']);
        $XIII = intval($body['XIII']);
        $XIV  = intval($body['XIV']);
        $XV   = intval($body['XV']);
        $XVI  = intval($body['XVI']);
        $XVII = intval($body['XVII']);

        $stmt = $mysqli->prepare("INSERT INTO respuestas 
            (I,II,III,IV,V,VI,VII,VIII,IX,X,XI,XII,XIII,XIV,XV,XVI,XVII) 
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");

        if(!$stmt) jerr("Error prepare: ".$mysqli->error,500);

        $stmt->bind_param(
            "ssissiiiiiiiiiiii",
            $body['I'],$body['II'],$III,$body['IV'],$V,$VI,$VII,$VIII,
    ...
    case 'update_respuesta': {
        ...
        $stmt = $mysqli->prepare("UPDATE respuestas SET 
            I=?,II=?,III=?,IV=?,V=?,VI=?,VII=?,VIII=?,IX=?,X=?,XI=?,XII=?,XIII=?,XIV=?,XV=?,XVI=?,XVII=? 
            WHERE numero=?");
        if(!$stmt) jerr("Error prepare: ".$mysqli->error,500);
        ...
        $body['numero'] = $numero;
        foreach (['III','V','VI','VII','VIII','IX','X','XI','XII','XIII','XIV','XV','XVI','XVII'] as $k) {
            $body[$k] = intval($body[$k]);
        }
        jok($body);
        break;
    }
